...
def aggregate_descriptors(descriptors, descriptor_filter, version_to_tag):
    ...
    for line in format_all_version_summaries(version_summaries):
        log.debug(line)
...
def format_all_version_summaries(version_summaries):
    return [
        '{}: [{}] power {} count {}'.format(version, summary.tag, summary.power, summary.count)
