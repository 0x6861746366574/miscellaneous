...
class SymbolLoader:
    def __init__(self, account_data_filepath, category_addresses_filepaths):
        self.account_data_filepath = account_data_filepath
        self.category_addresses_filepaths = category_addresses_filepaths

        self.descriptors = []

    def load(self):
        category_addresses_map = {}
        for category, addresses_filepath in self.category_addresses_filepaths.items():
        ...
        # load rows
        with open(self.account_data_filepath, 'r') as infile:
            csv_reader = csv.DictReader(infile, ['address', 'balance', 'is_voting', 'has_ever_voted', 'voting_end_epoch', 'version'])
    ...
    def aggregate(self, descriptor_filter=None):
        return aggregate_descriptors(self.descriptors, descriptor_filter, lambda version: 'for' if '1.0.3.0' == version else 'against')
